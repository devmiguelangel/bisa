	...
	if (($rowUser = $link->verify_type_user($_SESSION['idUser'], $_SESSION['idEF'])) !== false) {
		...
		case 'REP':
			...
			if (is_null($rowUser['u_depto']) === true 
					|| in_array($rowUser['u_usuario'], $user_rep) === true) {
				$data_subsidiary[] = array(
					'id'	=> '',
					'depto'	=> 'Todos',
					);
...
<h3 class="h3">Reportes de Cobranzas</h3>
<?php
$class = $display = '';
...
?>
		<td style="width:40%; border-bottom:1px solid #CECECE;">
        	<input type="hidden" id="flag" name="flag" value="<?=md5('RC');?>">
		</td>
	</tr>
</table>
<?php
...
?>
            ...
            <label style="">Días en Mora: </label>
            <select id="frp-mora" name="frp-mora" style="width: auto;">
				<option value="">Seleccione...</option>
<?php foreach ($link->days_mora as $key => $value): ?>
	<?php if ($key === 4): ?>
...
while($rowSt = $rsSt->fetch_array(MYSQLI_ASSOC)){
	echo '<label class="lbl-cb"><input type="checkbox" id="frp-estado-' 
		. $rowSt['id_estado'] . '" name="frp-estado-' . $rowSt['id_estado'] 
		. '" value="' . $rowSt['id_estado'] . '">&nbsp;' . $rowSt['estado'] . '</label> ';
}
$rsSt->free();
?>
                </div>
...
if ($rowMenu['producto'] === 'AP' || $rowMenu['producto'] === 'VI') {
?>
				<h5 style="display: none;">Estado de Cuenta</h5>
				<div style="display: none;">
<?php
	foreach ($link->state_account as $key => $value) {
		echo '<label class="lbl-cb">
			<input type="checkbox" id="frp-state-account-' . strtolower($key) . '" 
				name="frp-state-account-' . strtolower($key) . '" 
...
?>
                <h5>Estado</h5>
				<div>
<?php foreach ($link->state_account as $key => $value): ?>
					<label class="lbl-cb"><input type="checkbox" 
						id="frp-state-account-<?=strtolower($key);?>" 
						name="frp-state-account-<?=strtolower($key);?>" 
						value="<?=$key;?>">&nbsp;<?=$value;?></label>
<?php endforeach ?>
				</div>
