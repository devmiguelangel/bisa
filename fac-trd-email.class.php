	...
	public function send_mail_fac($ide, $emailCC = ''){
		...
		if(($this->rs = $this->cx->query($this->sql,MYSQLI_STORE_RESULT))){
			if($this->rs->num_rows === 1){
				...
				$mail->Subject = $subject.' a Caso Facultativo No. TRD-'.$this->row['no_emision'].' '.$arrCl[1];
				
				$_IMP = $this->cx->verify_implant(base64_encode($this->row['idef']), 'TRD');
				if ($_IMP === TRUE) {
					if (($rsUi = $this->cx->get_user_implant(base64_encode($this->row['u_id']), base64_encode($this->row['idef']), 'TRD')) !== FALSE) {
				...
				$this->sqlc = 'SELECT sc.correo, sc.nombre 
						...
						WHERE sc.producto = "TRD" AND sef.id_ef = "'.$this->row['idef'].'" AND sef.activado = true ;';
...
<div style="width:600px; border:1px solid #CCCCCC; color:#000000; font-weight:bold; font-size:12px; text-align:left;">
    ...
    <div style="padding:5px 10px;">TRD-<?=$this->row['no_emision'];?></div>
<?php
		if($state[0] === 'A' || $state[0] === 'R'){
?>
